		...
		protected override void Write(WpfVirtualizingRenderer renderer, EmphasisInline obj)
		{
			...
			if (obj.DelimiterChar == '*' || obj.DelimiterChar == '_')
			{
				if (obj.DelimiterCount >= 2)
					renderer.ApplyStyle(span, MarkdownStyles.StrongEmphasis);
			...
			if (renderer.CurrentContext is Span parentSpan)
				parentSpan.Inlines.Add(span);
			else if (renderer.CurrentContext is TextBlock parentTb)
				parentTb.Inlines.Add(span);
